    ...
    public function minhas(Request $request)
{
    try {
        $id = session('user.id');
        $query = Reserva::where('id_user', $id)
                        ->orderBy('ano_reserva', 'desc')
        ...
        if ($request->query('filtro') == 'ativas') {
            // Filtra reservas com data futura
            $today = now(); // Data de hoje
            $query->where(function ($query) use ($today) {
                $query->where('ano_reserva', '>', $today->year)
                      ->orWhere(function ($query) use ($today) {
                          $query->where('ano_reserva', $today->year)
                                ->where('mes_reserva', '>', $today->month)
                                ->orWhere(function ($query) use ($today) {
                                    $query->where('mes_reserva', $today->month)
                                          ->where('dia_reserva', '>', $today->day);
                                });
                      });
            });
        }
        ...
        return view('minhas', compact('reservas'));
    } catch (\Exception $e) {
